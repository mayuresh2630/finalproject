            ...
            <tbody>
              {tableData &&
                tableData.map((data, index) => (
                  <MatrixTableRow data={data} index={index} handleSkill={handleSkill}/>
                ))}
            </tbody>
          </table>
        </Typography>
...
function MatrixTableRow({ data, index,handleSkill}) {
  ...
  return (
    <>
      <tr key={index}>
        ...
        <td>
          <button style={{ height: "42px" }} onClick={() => setShowExtra(true)}>
            {showExtra ? (
